...
# ifdef  WAKE_EXTENSIONS
	if  ( wake_enabled )
	   {
		// Allow extra characters in function names
		while  ( * e  &&  ( isalnum ( ( int ) * e )  ||  strchr ( "<>.[]~-_&+=/%*|^!", * e )  !=  NULL ) )
			e ++ ;
...
/***
	...
	Made the function entry dynamic, to allow external source files to add their own function names.
	
	[Christian Vigh, 08/2015]
	Took into account the new alloc_fn field, which is set to 1 for functions defined with the define_new_function() function.
 ***/
function_table_entry 		function_table_init [] =
    {			   
	  FT_ENTRY 
	     ( 
		"abspath",        
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Returns the absolute path of each of the specified files." ),
			N_ ( "For each file name in 'names' return an absolute name that does not contain any . or .. components, "
			     "nor any repeated path separators (/).\n"
			     "Note that, in contrast to realpath function, abspath does not resolve symlinks and does not require "
			     "the file names to refer to an existing file or directory. Use the wildcard function to test for existence." ) 
		    )
	       ),
	 FT_ENTRY 
	    ( 
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "prefix,names..." ),
			N_ ( "Prepends a prefix to each name in the name list." ),
			N_ ( "The argument names is regarded as a series of names, separated by whitespace; prefix is used as a unit. "
			     "The value of prefix is prepended to the front of each individual name and the resulting larger names "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "suffix,names..." ),
			N_ ( "Appends a suffix to each name in the name list." ),
			N_ ( "The argument names is regarded as a series of names, separated by whitespace; suffix is used as a unit. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Extracts all but the suffix of each file name in names." ),
			N_ ( "Extracts all but the suffix of each file name in names. If the file name contains a period, the basename is "
			     "everything starting up to (and not including) the last period. Periods in the directory part are ignored.\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Extracts the directory-part of each file name in names." ),
			N_ ( "Extracts the directory-part of each file name in names. The directory-part of the file name is everything up " 
			     "through (and including) the last slash in it. If the file name contains no slash, the directory part is the string './'.\n"
			     "For example,\n"
			     "\t$(dir src/foo.c hacks)\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Extracts all but the directory-part of each file name in names." ),
			N_ ( "Extracts all but the directory-part of each file name in names. If the file name contains no slash, it is left unchanged. " 
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "from,to,text" ),
			N_ ( "Performs a textual replacement of 'from' with 'to' on the specified text." ),
			N_ ( "Performs a textual replacement on the text 'text' : each occurrence of 'from' is replaced by 'to'. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Extracts the suffix of each file name in 'names'." ),
			N_ ( "Extracts the suffix of each file name in names. If the file name contains a period, the suffix is "
			     "everything starting with the last period. Otherwise, the suffix is the empty string. This frequently means "
			     "that the result will be empty when names is not, and if names contains multiple file names, the result may contain "
			     "fewer file names.\n"
			     "For example,\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "pattern...,text" ),
			N_ ( "Returns all whitespace-separated words in text that do match any of the pattern words." ),
			N_ ( "Returns all whitespace-separated words in text that do match any of the pattern words, removing any words "
			     "that do not match. The patterns are written using '%', just like the patterns used in the patsubst function above.\n"
			     "The filter function can be used to separate out different types of strings (such as file names) in a variable.\n"
			     "For example:\n"
			     "\tsources := foo.c bar.c baz.s ugh.h\n"
			     "\tfoo: $(sources)\n"
			     "\t\tcc $(filter %.c %.s,$(sources)) -o foo\n"
			     "says that foo depends of foo.c, bar.c, baz.s and ugh.h but only foo.c, bar.c and baz.s should be specified in the "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "pattern...,text" ),
			N_ ( "Returns all whitespace-separated words in text that do not match any of the pattern words." ),
			N_ ( "Returns all whitespace-separated words in text that do not match any of the pattern words, removing the words "
			     "that do match one or more. This is the exact opposite of the filter function.\n"
			     "For example, given: \n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "find,in" ),
			N_ ( "Searches 'in' for an occurrence of 'find'." ),
			N_ ( "Searches 'in' for an occurrence of 'find'. If it occurs, the value is 'find'; otherwise, the value is empty. "
			     "You can use this function in a conditional to test for the presence of a specific substring in a given string. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Returns the first word in 'names'." ),
			N_ ( "The argument names is regarded as a series of names, separated by whitespace. The value is the first name "
			     "in the series. The rest of the names are ignored.\n"
			     "For example,\n"
			     "\t$(firstword foo bar)\n"
			     "produces the result 'foo'. Although $(firstword text) is the same as $(word 1,text), the firstword function is "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "variable" ),
			N_ ( "Returns the flavor of a variable." ),
			N_ ( "Returns the flavor of a variable. Note that variable is the name of a variable to inquire about; not a reference "
			     "to that variable.\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "list1,list2" ),
			N_ ( "Concatenates the two arguments word by word." ),
			N_ ( "Concatenates the two arguments word by word: the two first words (one from each argument) concatenated from the first "
			     "word of the result, the two second words form the second word of the result, and so on. So the nth word of the result "
			     "comes from the nth word of each argument. If one argument has more words that the other, the extra words are copied "
			     ...
			     "This function can merge the results of the dir and notdir functions, to produce the original list of files which was "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names..." ),
			N_ ( "Returns the last word in 'names'." ),
			N_ ( "The argument names is regarded as a series of names, separated by whitespace. The value is the last name in the series.\n"
			     "For example,\n"
			     "\t$(lastword foo bar)\n"
			     "produces the result 'bar'. Although $(lastword text) is the same as $(word $(words text),text), the lastword function was "
	...
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "pattern,replacement,text" ),
			N_ ( "Subtitutes a pattern with a replacement string in the specified text" ),
			N_ ( "Finds whitespace-separated words in text that match pattern and replaces them with replacement. Here pattern may contain a '%' "
			     "which acts as a wildcard, matching any number of any characters within a word. If replacement also contains a '%', the '%' is "
			     "replaced by the text that matched the '%' in pattern. Only the first '%' in the pattern and replacement is treated this way; "
			     "any subsequent '%' is unchanged.\n"
			     "'%' characters in patsubst function invocations can be quoted with preceding backslashes ('\'). Backslashes that would "
			     "otherwise quote '%' characters can be quoted with more backslashes. Backslashes that quote '%' characters or other backslashes "
			     "are removed from the pattern before it is compared file names or has a stem substituted into it. Backslashes that are not in "
			     "danger of quoting '%' characters go unmolested.\n"
			     "For example, the pattern the\\%weird\\\\%pattern\\\\ has 'the%weird\\' preceding the operative '%' character, and 'pattern\\\\' "
			     "following it. The final two backslashes are left alone because they cannot affect any '%' character.\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "names" ),
			N_ ( "For each file name in names return the canonical absolute name." ),
			N_ ( "For each file name in names return the canonical absolute name. A canonical name does not contain any . or .. components, "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "command [args]" ),
			N_ ( "Executes the specified shell command." ),
			N_ ( "The shell function performs the same function that backquotes ('`') perform in most shells: it does command expansion. "
			     "This means that it takes as an argument a shell command and evaluates to the output of the command. The only processing "
			     "make does on the result is to convert each newline (or carriage-return / newline pair) to a single space. If there is a "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "list" ),
			N_ ( "Sorts a list of words." ),
			N_ ( "Sorts the words of list in lexical order, removing duplicate words. The output is a list of words separated by single spaces. "
			     "Thus,\n"
			     "\t$(sort foo bar lose)\n"
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "string" ),
			N_ ( "Strips the specified string, removing leading and trailing whitespaces." ),
			N_ ( "Removes leading and trailing whitespace from string and replaces each internal sequence of one or more whitespace "
			     "characters with a single space. Thus, '$(strip a b c )' results in 'a b c'.\n" 
			     "The function strip can be very useful when used in conjunction with conditionals. When comparing something with the "
			     "empty string '' using ifeq or ifneq, you usually want a string of just whitespace to match the empty string." )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "pattern" ),
			N_ ( "Matches existing files using shell-like patterns ('*', '?' and '[...]')." ),
			N_ ( "The argument pattern is a file name pattern, typically containing wildcard characters (as in shell file name patterns). "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "n,text" ),
			N_ ( "Returns the nth word of 'text'." ),
			N_ ( "Returns the nth word of 'text'. The legitimate values of n start from 1. If n is bigger than the number of words in text, "
			     "the value is empty.\n"
			     "For example,\n"
			     "\t$(word 2, foo bar baz)\n"
			     "returns 'bar'." )
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "s,e,text" ),
			N_ ( "Returns the list of words in text starting with word s and ending with word e (inclusive)." ),
			N_ ( "Returns the list of words in text starting with word s and ending with word e (inclusive). "
			     "The legitimate values of 's' start from 1; 'e' may start from 0. If 's' is bigger than the number of words in text, "
			     "the value is empty. If 'e' is bigger than the number of words in text, words up to the end of text are returned. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "variable" ),
			N_ ( "Expands to the origin of a variable." ),
			N_ ( "Expands to the origin of a variable. Possible origins are :\n"
			     ". 'undefined'            \bif the variable was never defined.\n"	
			     ". 'default'              \bif the variable has a default definition, as is usual with CC and so on.\n"
			     ". 'environment'          \bif the variable was inherited from the environment provided to make.\n"
			     ". 'environment override' \bif the variable was inherited from the environment provided to make, and is overriding "
					"a setting for 'variable' in the makefile as a result of the '-e' option.\n"
			     ". 'file'                 \bif the variable was defined in a makefile.\n"
			     ". 'command line'         \bif the variable was defined on the command line.\n"
			     ". 'override'             \bif the variable was defined with an override directive in a makefile.\n"
			     ". 'automatic'            \bif the variable is an automatic variable defined for the execution of the recipe for each rule."
			    )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "var,list,text" ),
			N_ ( "Expands to 'text' for each 'var' in 'list'." ),
			N_ ( "Expands to 'text' for each 'var' in 'list'. For example,\n" 
			     "\tdirs := a b c\n"
			     "\tfiles := $(foreach dir,$(dirs),$(wildcard $(dir)/*))\n"
			     "will expand to : $(wildcard $(dir)/a $(wildcard $(dir)/b $(wildcard $(dir)/c" )
 		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "variable,params..." ),
			N_ ( "Calls a user-defined function using the specified parameters." ),
			N_ ( "Calls a user-defined function using the specified parameters. User-defined functions are special macros that "
			     "can accept parameters, such as :\n"
			     "\treverse = $(2) $(1)\n"
			     "The following call :\n"
			     "\tfoo = $(call reverse,a,b)\n"
			     "will make 'foo' equal to 'b a'." 
			     )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "text.." ),
			N_ ( "Prints its expanded arguments to standard output." ),
			N_ ( "This function does nothing more than print its (expanded) argument(s) to standard output. No makefile name or "
			     "line number is added. The result of the expansion of this function is the empty string."  )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "text..." ),
			N_ ( "Displays the expanded text to standard error." ),
			N_ ( "This function works similarly to the error function, above, except that make doesn't exit. Instead, text is expanded "
			     "and the resulting message is displayed, but processing of the makefile continues. The result of the expansion of this "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "text..." ),
			N_ ( "Generates a fatal error where the message is the expanded value of 'text'." ),
			N_ ( "Generates a fatal error where the message is 'text'. Note that the error is generated whenever this function is evaluated. "
	...
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "condition,then-part[,else-part]" ),
			N_ ( "The if function provides support for conditional expansion in a functional context." ),
			N_ ( "The if function provides support for conditional expansion in a functional context (as opposed to the GNU make makefile "
			     "conditionals such as ifeq.\n"
			     "The first argument, condition, first has all preceding and trailing whitespace stripped, then is expanded. If it expands "
			     "to any non-empty string, then the condition is considered to be true. If it expands to an empty string, the condition is "
			     "considered to be false.\n" 
			     "If the condition is true then the second argument, 'then-part', is evaluated and this is used as the result of the "
			     "evaluation of the entire if function. "
			     ...
			     "Note that only one of the then-part or the else-part will be evaluated, never both. Thus, either can contain side-effects "
	...
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "condition1,[condition2,[condition3,[...]]]" ),
			N_ ( "Simulates the boolean OR operator." ),
			N_ ( "The or function provides a \"short-circuiting\" OR operation. Each argument is expanded, in order. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "condition1,[condition2,[condition3,[...]]]" ),
			N_ ( "Simulates the boolean AND operator." ),
			N_ ( "The and function provides a 'short-circuiting' AND operation. Each argument is expanded, in order. If an argument expands "
			     "to an empty string the processing stops and the result of the expansion is the empty string. If all arguments expand to a "
			     "non-empty string then the result of the expansion is the expansion of the last argument." )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "variable" ),
			N_ ( "Allows for using the value of a variable without having it expanded." ),
			N_ ( "The value function provides a way for you to use the value of a variable without having it expanded. "
			     "Please note that this does not undo expansions which have already occurred; for example if you create a simply expanded "
			     "variable its value is expanded during the definition; in that case the value function will return the same result as using "
			     ...
			     "The result of this function is a string containing the value of variable, without any expansion occurring. "
			     "For example, in this makefile :\n"
			     "\tFOO = $PATH\n\n"
			     "\tall :\n"
			     "\t\t@echo $(FOO)\n"
			     "\t\t@echo $(value FOO)\n"
			     "The first output line would be ATH, since the \"$P\" would be expanded as a make variable, while the second output line would be "
			     "the current value of your $PATH environment variable, since the value function avoided the expansion." )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "expression" ),
			N_ ( "Evaluates an expression. The result can be a variable or a rule, for example." ),
			N_ ( "The eval function is very special: it allows you to define new makefile constructs that are not constant; which are the result "
			     "of evaluating other variables and functions. The argument to the eval function is expanded, then the results of that expansion "
			     "are parsed as makefile syntax. The expanded results can define new make variables, targets, implicit or explicit rules, etc.\n"
			     "The result of the eval function is always the empty string; thus, it can be placed virtually anywhere in a makefile without "
			     "causing syntax errors.\n"
			     "It's important to realize that the eval argument is expanded twice; first by the eval function, then the results of that expansion "
			     "are expanded again when they are parsed as makefile syntax. This means you may need to provide extra levels of escaping for \"$\" "
			     "characters when using eval. The value function can sometimes be useful in these situations, to circumvent unwanted expansions.\n" 
			     "Here is an example of how eval can be used; this example combines a number of concepts and other functions. "
			     "Although it might seem overly complex to use eval in this example, rather than just writing out the rules, consider two things: "
			     "first, the template definition (in PROGRAM_template) could need to be much more complex than it is here; and second, you might put "
			     "the complex, \"generic\" part of this example into another makefile, then include it in all the individual makefiles. "
	   ...
	   (
		...
		FUNC_HELP
		   (
			N_ ( "builtins" ),
			N_ ( "value1,value2" ),
			N_ ( "Evaluates to '1' if both 'value1' and 'value2' are equal, or to the empty string (false) otherwise." ),
			N_ ( "Evaluates to '1' if both 'value1' and 'value2' are equal, or to the empty string (false) otherwise." )
		    )
	     ),
	FT_ENTRY
	   (
		...
		FUNC_HELP
		   (
			...
			N_ ( "value" ),
			N_ ( "Negates a value." ),
			N_ ( "Expands to '1' if 'value' is an empty string, or to an empty string otherwise." )
		    )
	     )
# endif
};
...
/***
	[Christian Vigh, 09/2013]
	Introduced this function to be able to load builtin function definitions from multiple sources (the
	main one being make, the second one the wake extensions).
 ***/
function_table_entry *		builtin_functions			[ FUNCTABLE_MAX_BUILTINS ] ;
unsigned int			builtin_function_count	 =  0 ;


void	hash_load_functions ( function_table_entry *  item_table, unsigned long  cardinality )
   {
	int				i ;
	function_table_entry *		p,
			     **		q ;

	// Make some sanity checkings
	if  ( cardinality + builtin_function_count  >=  FUNCTABLE_MAX_BUILTINS )
		OS ( error, NILF, "Too many builtin functions defined. Please redefine the FUNCTABLE_MAX_BUILTINS constant in function.h to a higher value.", NULL ) ;

	// Save pointers to the initial items
	for  ( p = item_table, q = builtin_functions + builtin_function_count, i = cardinality ; i ; p ++, q ++, i -- )
		* q = p ;

	// Load the definitions into the hash table
	hash_load  ( & function_table, item_table, cardinality, sizeof (struct function_table_entry) ) ;

	// Upgrade the counter
	builtin_function_count += cardinality ;
    }
# endif
...
hash_init_function_table (void)
{
  ...
  hash_load_functions (function_table_init, FUNCTABLE_SIZE ( function_table_init ) ) ;
