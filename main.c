...
# ifdef		WAKE_EXTENSIONS
    N_("\nWake extensions options :\n" ),
    N_("\
  --stdmake                             Run as the normal make command (disable wake extensions).\n"),
    N_("\
  --builtins[=category]			Displays a list of available builtin functions.\n" ),
    N_("\
  --builtin-categories                  Displays a list of available builtin categories.\n" ),
    N_("\
  --builtin-help[=name]			Displays help text for the specified builtin function name, or for all functions.\n" ),
    N_("\
  --builtin-signatures[=category]	Displays a list of available builtin functions, with their signature.\n" ),
    N_("\
  --builtin-usage[=name]		Displays a usage text for the specified builtin function name, or for all functions.\n" ),
# endif
...
# ifdef		WAKE_EXTENSIONS
	{ CHAR_MAX + 100, strlist, & wake_cmdline_options. print_builtins	, 0, 0, 0, "", 0, "builtins" },
	{ CHAR_MAX + 101, strlist, & wake_cmdline_options. print_signatures	, 0, 0, 0, "", 0, "builtin-signatures" },
	{ CHAR_MAX + 102, strlist, & wake_cmdline_options. print_help		, 0, 0, 0, "", 0, "builtin-help" },
	{ CHAR_MAX + 103, strlist, & wake_cmdline_options. print_usage		, 0, 0, 0, "", 0, "builtin-usage" },
	{ CHAR_MAX + 104, flag   , & wake_cmdline_options. print_categories	, 0, 0, 0, "", 0, "builtin-categories" },
    { CHAR_MAX + 105, flag   , & wake_cmdline_options. standard_make        , 0, 0, 0, "", 0, "stdmake" },
# endif
    { 0, 0, 0, 0, 0, 0, 0, 0, 0 }
  };
...
# endif
  ...
  if (ISDB (DB_BASIC))
    print_version ();
# ifdef		WAKE_EXTENSIONS
	wake_process_cmdline ( & wake_cmdline_options ) ;

	if  ( wake_enabled )
		wake_initialize ( ) ;

# endif		/* WAKE_EXTENSION */
...
# ifdef		WAKE_EXTENSIONS
  if (!remote_description || *remote_description == '\0')
    fprintf (usageto, _("\nThis program was built for %s\n"), make_host);
  else
    fprintf (usageto, _("\nThis program was built for %s (%s)\n"),
             make_host, remote_description);

  fprintf (usageto, _("Report bugs to <bug-make@gnu.org>\n"));
  fprintf (usageto, _("Report bugs in Wake extensions to <christian.vigh@orange.fr>\n"));
# else
  if (!remote_description || *remote_description == '\0')
...
# ifdef		WAKE_EXTENSIONS
	printf ( "%s\n%sWake extensions v%s by %s, %s.\n",
			precede, precede,
			WAKE_EXTENSIONS_VERSION,
			WAKE_EXTENSIONS_AUTHOR,
			WAKE_EXTENSIONS_UPDATE
		) ;
# endif
